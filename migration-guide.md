...
The [`max`](query-builder/executing-queries/aggregates.md#max), [`min`](query-builder/executing-queries/aggregates.md#min), [`count`](query-builder/executing-queries/aggregates.md#count), and [`sum`](query-builder/executing-queries/aggregates.md#sum) methods now accept a `defaultValue` argument.  This comes **before** the `defaultOptions` argument.  If you are using positional parameters with any of these functions, update your code to the new method signature.
